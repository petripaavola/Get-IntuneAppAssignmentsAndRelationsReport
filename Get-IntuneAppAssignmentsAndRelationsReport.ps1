...
.TAGS Intune Application Assignment and Relationship Report

.LICENSEURI

.PROJECTURI https://github.com/petripaavola/Get-IntuneAppAssignmentsAndRelationsReport
...
.DESCRIPTION
   ...
   Changelog v4.0
   - Script uses Microsoft Graph Authentication module
   - Added Detection Check information to Application hover-on ToolTip
   ...
   You can work almost with cached data only without network connection with parameter -UseOfflineCache
   However EntraID Group membership counts are always downloaded from Graph API to get correct information
...
.OUTPUTS
   Creates report files
    ...
    ./yyyyMMddHHmmss_Intune_Application_Relationship_report.html
   ...
   Creates cache files
    ...
    ./cache/MobileAppRelationships.json
	./cache/appicons
...
function Add-EntraIDGroupGroupTypeExtraProperties {
	Param(
	...
	#$EntraIDGroups | ConvertTo-Json -Depth 5 | Set-Clipboard
	...
	foreach($group in $EntraIDGroups) {
		...
		# Check if group is directoryRole which is not actual EntraID Group
		if($group.'@odata.type' -eq '#microsoft.graph.directoryRole') {
			# Group is NOT security group at all
			# DirectoryRoles are not EntraID groups
...
function Add-EntraIDGroupDevicesAndUserMemberCountExtraProperties {
	Param(
		[Parameter(Mandatory=$true,
			ValueFromPipeline=$true,
			ValueFromPipelineByPropertyName=$true, 
	...
	Write-Verbose "Getting EntraID groups membercount for $($EntraIDGroups.Count) groups"

	for ($i=0; $i -lt $EntraIDGroups.count; $i+=20){
		...
		# For-loop will end automatically when loop counter is same as total count of $EntraIDGroups
		for ($a=$i; (($a -lt $i+20) -and ($a -lt $EntraIDGroups.count)); $a+=1) {

			if(($EntraIDGroups[$a]).'@odata.type' -eq '#microsoft.graph.directoryRole') {
				# Azure DirectoryRole is not EntraID Group
				$GraphAPIBatchEntry_DevicesCount = @{
					id = ($a+1).ToString()
					"method" = "GET"
					"url" = "/directoryRoles/$(($EntraIDGroups[$a]).id)"
				}

			} else {
				# We should have EntraID Group
				$GraphAPIBatchEntry_DevicesCount = @{
					id = ($a+1).ToString()
					"method" = "GET"
					"url" = "/groups/$(($EntraIDGroups[$a]).id)/transitivemembers/microsoft.graph.device/`$count?ConsistencyLevel=eventual"
				}
			}
			...
			if(($EntraIDGroups[$a]).'@odata.type' -eq '#microsoft.graph.directoryRole') {
				...
				$GraphAPIBatchEntry_UsersCount = @{
					id = ($a+1).ToString()
					"method" = "GET"
					"url" = "/groups/$(($EntraIDGroups[$a]).id)/transitivemembers/microsoft.graph.user/`$count?ConsistencyLevel=eventual"
				}
			}
			...
			#Write-Host "`$a=$a   `$i=$i    GroupName=$($EntraIDGroups[$a].displayName)"
		}
		...
		$MgGraphRequest = Invoke-MgGraphRequest -Uri $url -Body $requests_devices_count_JSON.ToString() -Method 'POST' -OutputType PSObject
		$Success = $?
		...
		if($Success) {
			...
			Write-Error "Error getting EntraID groups devices count"
			return 1
		}
		...
		$EntraIDGroups_Devices_MemberCount_Batch_Result = Get-MSGraphAllPages -SearchResult $MgGraphRequest
		$Success = $?
		...
		$EntraIDGroups_Devices_MemberCount_Batch_Result = $MgGraphRequest

		# DEBUG
		#$EntraIDGroups_Devices_MemberCount_Batch_Result

		# Process results for devices count batch requests
		Foreach ($response in $EntraIDGroups_Devices_MemberCount_Batch_Result.responses) {
			$GroupArrayIndex = $response.id - 1
			if($response.status -eq 200) {
				
				if(($EntraIDGroups[$GroupArrayIndex]).'@odata.type' -eq '#microsoft.graph.directoryRole') {
					# DEBUG
					#Write-Verbose "EntraID directoryRole (arrayIndex=$GroupArrayIndex) $($EntraIDGroups[$GroupArrayIndex].displayName)"

					$EntraIDGroups[$GroupArrayIndex] | Add-Member -MemberType noteProperty -Name YodamiittiCustomGroupMembersCountDevices -Value 'N/A'
				} else {
					# DEBUG
					#Write-Verbose "EntraID group (arrayIndex=$GroupArrayIndex) $($EntraIDGroups[$GroupArrayIndex].displayName) adding devices count property: $($response.body)"
					
					$EntraIDGroups[$GroupArrayIndex] | Add-Member -MemberType noteProperty -Name YodamiittiCustomGroupMembersCountDevices -Value $response.body					
				}
			} else {
				Write-Error "Error getting devices count for EntraID group $($EntraIDGroups[$GroupArrayIndex].displayName)"
		...
		$url = 'https://graph.microsoft.com/beta/$batch'
		$MgGraphRequest = Invoke-MgGraphRequest -Uri $url -Body $requests_users_count_JSON.ToString() -Method 'POST' -OutputType PSObject
		$Success = $?
		...
		if($Success) {
			...
			# Invoke-MSGraphRequest failed
			Write-Error "Error getting EntraID groups users count"
			return 1
		}
		...
		# Get AllMSGraph pages
		# This is also workaround to get objects without assigning them from .Value attribute
		$EntraIDGroups_Users_MemberCount_Batch_Result = Get-MSGraphAllPages -SearchResult $MgGraphRequest
		$Success = $?
...
#>
		...
		$EntraIDGroups_Users_MemberCount_Batch_Result = $MgGraphRequest

		# DEBUG
		#$EntraIDGroups_Users_MemberCount_Batch_Result

		# Process results for devices count batch requests
		Foreach ($response in $EntraIDGroups_Users_MemberCount_Batch_Result.responses) {
			$GroupArrayIndex = $response.id - 1
			if($response.status -eq 200) {
				...
				if(($EntraIDGroups[$GroupArrayIndex]).'@odata.type' -eq '#microsoft.graph.directoryRole') {
					...
					# Change "EntraID Group" json to actual real directoryRole json which we just got from batch request
					$EntraIDGroups[$GroupArrayIndex] = $response.body
					
					$EntraIDGroups[$GroupArrayIndex] | Add-Member -MemberType noteProperty -Name YodamiittiCustomGroupMembersCountUsers -Value 'N/A'
					
					# We need to add below properties again because we just replace whole object so we lost earlier customProperties
					$EntraIDGroups[$GroupArrayIndex] | Add-Member -MemberType noteProperty -Name YodamiittiCustomGroupMembersCountDevices -Value 'N/A' -Force
					$EntraIDGroups[$GroupArrayIndex] | Add-Member -MemberType noteProperty -Name YodamiittiCustomGroupType -Value 'DirectoryRole' -Force
				} else {
					# DEBUG
					#Write-Verbose "EntraID group (arrayIndex=$GroupArrayIndex) $($EntraIDGroups[$GroupArrayIndex].displayName) adding users count property: $($response.body)"
					
					$EntraIDGroups[$GroupArrayIndex] | Add-Member -MemberType noteProperty -Name YodamiittiCustomGroupMembersCountUsers -Value $response.body
				}
				
				
			} else {
				Write-Error "Error getting users count for EntraID group $($EntraIDGroups[$GroupArrayIndex].displayName)"
				Write-Error "$($response | ConvertTo-Json)"
			}
		}
...
try {
    ...
    if (-not ($UseOfflineCache)) {
        try {
			...
			Write-Host "Check required PowerShell modules are available before importing and show their versions"
			$ModulesAvailable += Get-Module -ListAvailable $PowerShellModulesRequiredList
			$ModulesAvailableText = $ModulesAvailable | Format-Table Name, Version | Out-String

			Write-Host "Modules available:"
			Write-Host "$ModulesAvailableText"
			Write-Host ""
			...
			Function Import-PowershellModule {
				...
				if ($Module) {
					...
					Try {
						$ImportedModule = Import-Module -Name $Module.Name -PassThru -ErrorAction Ignore
						$Success = $?
					} catch {
						Write-Host "Fatal error importing $($Module.Name) module!`n$($_.Exception.Message)"
						Write-Host "Scripting will fail and exit 45"
						Exit 45
					}

					if($Success -and $ImportedModule) {
						Write-Host "$($ImportedModule.Name) module version $($ImportedModule.Version) found and imported successfully" -ForegroundColor Green
						Write-Host ""
					} else {
						Write-Host "Failed to import $($Module.Name) module!" -ForegroundColor Red
						Write-Host "Please install the module with command:"
						Write-Host "Install-Module $($ModuleName) -Scope CurrentUser" -ForegroundColor Yellow
						Write-Host "Script will fail and exit 46"
						Exit 46
					}
				} else {
					Write-Host "$($ModuleName) module not found! Please install the module with command:" -ForegroundColor Red
					Write-Host "Install-Module $($ModuleName) -Scope CurrentUser" -ForegroundColor Yellow
					Write-Host "Script will fail and exit 44"
					exit 44
				}
			}

			# Import required PowerShell modules
			Write-Host "Import required PowerShell modules for Microsoft Graph API connection"
			Write-Host ""
			$PowerShellModulesRequiredList | ForEach-Object {
				Import-PowershellModule -ModuleName $_
			}
			Write-Host ""
			...
			Write-Host "Error connecting to Intune tenant!`nScript will exit!" -ForegroundColor Red
		}
    } else {
	...
	try {
		...
		if (-not ($UseOfflineCache)) {
			...
			$AllIntuneFilters = Get-MgBetaDeviceManagementAssignmentFilter -All
			$Success = $?
    ...
    ###################################################
	## MARK: Get Groups
    # Get EntraIDGroups. This should be more efficient than getting EntraIDGroup for every assignment one by one

    # Test if we have AllGroups.json file
    if (-not (Test-Path "$PSScriptRoot\cache\AllGroups.json")) {
        Write-Host "Did NOT find AllGroups.json file. We have to get EntraID Group information from Graph API"
        if ($UseOfflineCache) {
    ...
    try {
        if (-not ($UseOfflineCache)) {
            Write-Host "Downloading all EntraID Security Groups from Graph API (this might take a while)..."
            Write-Verbose "Downloading all EntraID Security Groups from Graph API"
            ...
            if (-not ($Success)) {
                Write-Error "Error downloading EntraID Security Groups"
                Write-Host "Script will exit..."
                Pause
                Exit 1
            }

            Write-Host "Found $($AllGroups.Count) EntraID Groups." -ForegroundColor Green
            ...
            Write-Host "Using cached EntraID Group information from file: $PSScriptRoot\cache\AllGroups.json"
        }
        ...
        Write-Error "$($_.Exception.GetType().FullName)"
        Write-Error "$($_.Exception.Message)"
        Write-Error "Error trying to download EntraID Group information"
        Write-Host "Script will exit..."
        Pause
        Exit 1
    }
    ...
    ###################################################
	## MARK: Get Apps
	# Get All Intune Apps with Assignments
    ...
    try {
        ...
        if (-not ($UseOfflineCache)) {
			...
			# This does NOT return Assignments property's all sub-properties! :(
			#$AllApps = Get-MgBetaDeviceAppManagementMobileApp -ExpandProperty assignments -All
            $Success = $?
			...
			# Workaround to get all properties from Assignments property
            ...
            Write-Host "Found $($AllApps.Count) Intune Applications" -ForegroundColor Green

            # Save to local cache
            # Really important parameter!!! Specify -Depth 5 because Application assignment data will be nested down 5 levels
            $AllApps | ConvertTo-Json -Depth 5 | Out-File "$PSScriptRoot\cache\AllApps.json" -Force
	...
	# Test if we have MobileAppRelationships.json file
    if (-not (Test-Path "$PSScriptRoot\cache\MobileAppRelationships.json")) {
        Write-Host "Could NOT find MobileAppRelationships.json file. We have to get Apps information from Graph API"
        if ($UseOfflineCache) {
            Write-Host "Run script without option -UseOfflineCache to download necessary MobileAppRelationships information`n" -ForegroundColor "Yellow"
            Exit 0
        }
    }
    ...
    try {
        ...
        if (-not ($UseOfflineCache)) {
            # Get App information from Graph API
            Write-Host "Downloading Intune mobile app relationships information from Graph API"

			$uri = 'https://graph.microsoft.com/beta/deviceAppManagement/mobileAppRelationships'
			$AllMobileAppRelationships = @()

			do {
				$response = Invoke-MgGraphRequest -Method GET -Uri $uri
				$AllMobileAppRelationships += $response.value
				$uri       = $response.'@odata.nextLink'
			} while ($uri)

            Write-Host "Found $($AllMobileAppRelationships.Count) Intune mobile app relationships" -ForegroundColor Green

            # Save to local cache
            # Really important parameter!!! Specify -Depth 5 because Application assignment data will be nested down 5 levels
            $AllMobileAppRelationships | ConvertTo-Json -Depth 5 | Out-File "$PSScriptRoot\cache\MobileAppRelationships.json" -Force

        }
        else {
            Write-Host "Using cached Intune mobile app relationships information from file: $PSScriptRoot\cache\MobileAppRelationships.json"
        }

        # Get App information from cached file always
        $AllMobileAppRelationships = Get-Content "$PSScriptRoot\cache\MobileAppRelationships.json" | ConvertFrom-Json
		
		# Create Hashtable for quicker search
		$AllMobileAppRelationshipsHashTable = @{}
		$AllMobileAppRelationships | Foreach-Object { $id = $_.id; $value=$_; $AllMobileAppRelationshipsHashTable["$id"] = $value }

    }
    catch {
        Write-Error "$($_.Exception.GetType().FullName)"
        Write-Error "$($_.Exception.Message)"
        Write-Error "Error trying to download Intune mobile app relationships information"
        Write-Host "Script will exit..."
        Pause
        Exit 1
    }
	...
	# Create custom object array to list which EntraID Groups had assignments
	# We will get more information for these groups only
	$EntraIDGroupsWithAssignments = @()
    ...
    try {
        ...
        Foreach ($App in $AllApps) {
            ...
            Foreach ($Assignment in $App.Assignments) {
				...
				Write-Verbose "Processing assignment: $($assignmentId) Intent: $($assignmentIntent)"

				# Skip if we do not have assignmentId or assignmentIntent
				if(-not $assignmentId) {
					Continue
				}

				# Skip if we do not have assignmentIntent
				if(-not $assignmentIntent) {
					Continue
				}
				...
				Try {
					...
					Write-Host "Error processing assignment intent for $($assignmentId)" -ForegroundColor Red
					Pause
					Exit 1
				}
				...
				# Builtin All Users and All Devices are not real EntraID Groups so they don't exist in Hashtable
				...
				# Add group to another array so we can get Devices and Users count to each EntraID Group
                $EntraIDGroupsWithAssignments += $assignmentTargetGroupObject
		...
		# Remove duplicate EntraIDGroups
		$EntraIDGroupsWithAssignments = $EntraIDGroupsWithAssignments | Sort-Object -Property id -Unique
		
		# Add groupType information
		$EntraIDGroupsWithAssignments = Add-EntraIDGroupGroupTypeExtraProperties $EntraIDGroupsWithAssignments
		
		# Get count of Devices and Users for AssignmentGroups
		$EntraIDGroupsWithAssignments = Add-EntraIDGroupDevicesAndUserMemberCountExtraProperties $EntraIDGroupsWithAssignments

		# Create Hashtable for quicker search
		$EntraIDGroupsWithAssignmentsHashTable = @{}
		$EntraIDGroupsWithAssignments | Foreach-Object { $id = $_.id; $value=$_; $EntraIDGroupsWithAssignmentsHashTable["$id"] = $value }
		...
		Foreach ($AppAssignment in $AppsWithAssignmentInformation) {
			if($AppAssignment.assignmentTargetGroupId) {
				if($EntraIDGroupsWithAssignmentsHashTable.ContainsKey($AppAssignment.assignmentTargetGroupId)) {
					$AppAssignment.devices = $EntraIDGroupsWithAssignmentsHashTable[$AppAssignment.assignmentTargetGroupId].YodamiittiCustomGroupMembersCountDevices
					$AppAssignment.users = $EntraIDGroupsWithAssignmentsHashTable[$AppAssignment.assignmentTargetGroupId].YodamiittiCustomGroupMembersCountUsers
				}
			}
		}
		...
		if($ExportCSV -or $ExportJSON -or $ExportToExcelCopyPaste -or $Passthru) {
			...
			# This does not work!
			#$EntraIDGroupsWithAssignmentsForExport = $AppsWithAssignmentInformation
			
			# Deep copy array of objects (this does not work for nested objects)
			$EntraIDGroupsWithAssignmentsForExport = $AppsWithAssignmentInformation | ForEach-Object { $_.psobject.Copy() }

		}
		
		# DEBUG - copy to clipboard
		#$EntraIDGroupsWithAssignments | ConvertTo-Json -Depth 5 | Set-Clipboard
		...
		Foreach($AppAssignment in $AppsWithAssignmentInformation) {
			...
			if($AppAssignment.id -and ($AllAppsHashTable.ContainsKey($AppAssignment.id))) {
				...
				# Detection rule info (minimal change, custom script in clear text)
				if ($AppObject.detectionRules) {
					$AppToolTip += "<br><strong>Detection Rules:</strong><br>"
					foreach ($rule in $AppObject.detectionRules) {
						$type = $rule.'@odata.type'
						if ($type -eq '#microsoft.graph.win32LobAppPowerShellScriptDetection') {
							# Show custom script in clear text
							$scriptContent = $rule.scriptContent
							if ($scriptContent) {
								try {
									$decoded = [System.Text.Encoding]::UTF8.GetString([System.Convert]::FromBase64String($scriptContent))
									$AppToolTip += "<pre>Custom Script:<br>" + [System.Web.HttpUtility]::HtmlEncode($decoded) + "</pre>"
								} catch {
									$AppToolTip += "<pre>Custom Script (Base64):<br>" + [System.Web.HttpUtility]::HtmlEncode($scriptContent) + "</pre>"
								}
							}
						} else {
							# Show other detection rule types minimally
							$AppToolTip += "Type: $type<br>"
							foreach ($prop in $rule.PSObject.Properties) {
								if ($prop.Name -ne '@odata.type' -and $prop.Value) {
									$AppToolTip += "$($prop.Name): $($prop.Value)<br>"
								}
							}
						}
					}
			...
			[String]$EntraIDGroupToolTip = ""

			if((-not $AppAssignment.assignmentTargetGroupId) -and ($AppAssignment.assignmentTargetGroupDisplayName -eq 'All Users')) {
				$EntraIDGroupToolTip = "Intune Built-in <strong>All Users</strong> group"
				
				$AppAssignment.assignmentTargetGroupDisplayName = "<!-- $($AppAssignment.assignmentTargetGroupDisplayName) --><a href=`"#`" class=`"tooltip`">$($AppAssignment.assignmentTargetGroupDisplayName)<span class=`"tooltiptext`">$EntraIDGroupToolTip</span></a>"
			}
			
			if((-not $AppAssignment.assignmentTargetGroupId) -and ($AppAssignment.assignmentTargetGroupDisplayName -eq 'All Devices')) {
				$EntraIDGroupToolTip = "Intune Built-in <strong>All Devices</strong> group"
				
				$AppAssignment.assignmentTargetGroupDisplayName = "<!-- $($AppAssignment.assignmentTargetGroupDisplayName) --><a href=`"#`" class=`"tooltip`">$($AppAssignment.assignmentTargetGroupDisplayName)<span class=`"tooltiptext`">$EntraIDGroupToolTip</span></a>"
			}
			
			# Builtin All Users and All Devices are not real EntraID Groups so they don't have group id value
			if($AppAssignment.assignmentTargetGroupId) {
				
				$EntraIDGroupToolTip += "<h3>$($AppAssignment.assignmentTargetGroupDisplayName)</h2>"
				
				$description = $EntraIDGroupsWithAssignmentsHashTable[$AppAssignment.assignmentTargetGroupId].description
				$EntraIDGroupToolTip +=	"Description: $description<br><br>"
				
				$groupType = "$($EntraIDGroupsWithAssignmentsHashTable[$AppAssignment.assignmentTargetGroupId].YodamiittiCustomMembershipType) $($EntraIDGroupsWithAssignmentsHashTable[$AppAssignment.assignmentTargetGroupId].YodamiittiCustomGroupType)"
				$EntraIDGroupToolTip += "<strong>$groupType</strong> EntraID group<br><br>"
				
				# Add Dynamic membershipRule if it exists
				if($EntraIDGroupsWithAssignmentsHashTable[$AppAssignment.assignmentTargetGroupId].membershipRule) {
					$membershipRule = $EntraIDGroupsWithAssignmentsHashTable[$AppAssignment.assignmentTargetGroupId].membershipRule
					$EntraIDGroupToolTip += "DynamicRule: <strong>$membershipRule</strong><br><br>"
				}					
												
				$devicesCount = $EntraIDGroupsWithAssignmentsHashTable[$AppAssignment.assignmentTargetGroupId].YodamiittiCustomGroupMembersCountDevices
				$usersCount = $EntraIDGroupsWithAssignmentsHashTable[$AppAssignment.assignmentTargetGroupId].YodamiittiCustomGroupMembersCountUsers
				$EntraIDGroupToolTip += "$devicesCount devices, $usersCount users"
				
				$AppAssignment.assignmentTargetGroupDisplayName = "<!-- $($AppAssignment.assignmentTargetGroupDisplayName) --><a href=`"https://intune.microsoft.com/#view/Microsoft_AAD_IAM/GroupDetailsMenuBlade/~/Overview/groupId/$($AppAssignment.assignmentTargetGroupId)`" target=`"_blank`" class=`"tooltip`">$($AppAssignment.assignmentTargetGroupDisplayName)<span class=`"tooltiptext`">$EntraIDGroupToolTip</span></a>"
			}
        ...
        Write-Host "Error creating Application custom object" -ForegroundColor Red
		Write-Host ""
		Write-Error "$($_.Exception.GetType().FullName)"
        Write-Error "$($_.Exception.Message)"
        Write-Host "Script will exit..."
        ...
        foreach ($ManagedApp in $AppIconDownloadList) {
            ...
            if ($IconFileObject -and (-not ($UpdateIconsCache))) {
                ...
                try {
					...
					$appLargeIcon = Invoke-MgGraphRequest -Uri $url -Method GET
    ...
    try {
		...
		<header>
		  ...
		  <div class="additional-info">
			...
			<p><strong>by</strong> $($AdminUserUPN)</p>
			<!-- <p><strong>Tenant name:</strong> $TenantDisplayName</p> -->
			<!-- <p><strong>Tenant id:</strong> $($TenantId)</p> -->
		  </div>
		  <div class="author-info">
			<p><a href="https://github.com/petripaavola/Get-IntuneAppAssignmentsAndRelationsReport" target="_blank"><strong>Download Report tool from GitHub</strong></a><br>Author: Petri Paavola - Microsoft MVP</p>
		  </div>
		</header>
		<br>
"@
		...
		<footer>
			...
			<p style="margin: 0;">
			  <a href="https://github.com/petripaavola/Get-IntuneAppAssignmentsAndRelationsReport" target="_blank"><strong>Download report tool from GitHub</strong<</a>
			</p>
		</footer>
		<script>
...
'@
    ...
    Write-Host "Uups! Something happened and we failed. Try again..." -ForegroundColor Red
	Write-Host ""
...
###########################################################
## MARK: Relationship
# Create application relationship report


$IntuneAppRelationshipJSONFile = "$PSScriptRoot\cache\MobileAppRelationships.json"
$InIntuneAppsFile = "$PSScriptRoot\cache\AllApps.json"
$OutFile = "$($ReportRunDateFileName)_Intune_Application_Relationship_report.html"


  function Test-ProblematicDetection {
    param($srcApp, $tgtApp)
    $srcRules = Get-DetectionRules $srcApp
    $tgtRules = Get-DetectionRules $tgtApp
    foreach ($src in $srcRules) {
      foreach ($tgt in $tgtRules) {
        $isFile = $src.'@odata.type' -eq '#microsoft.graph.win32LobAppFileSystemDetection' -and $tgt.'@odata.type' -eq '#microsoft.graph.win32LobAppFileSystemDetection'
        $isReg = $src.'@odata.type' -eq '#microsoft.graph.win32LobAppRegistryDetection' -and $tgt.'@odata.type' -eq '#microsoft.graph.win32LobAppRegistryDetection'
        if ($isFile) {
          $srcName = Normalize-FileName $src.fileOrFolderName
          $tgtName = Normalize-FileName $tgt.fileOrFolderName
          Write-Verbose ("Comparing file src: '{0}' tgt: '{1}'" -f $srcName, $tgtName)
          if ($srcName -and $tgtName -and $srcName -eq $tgtName) {
            $srcType = ($src.detectionType -as [string])
            $tgtType = ($tgt.detectionType -as [string])
            $srcOp = ($src.operator -as [string])
            $tgtOp = ($tgt.operator -as [string])
            if ($srcType) { $srcTypeNorm = $srcType.Trim().ToLower() } else { $srcTypeNorm = '' }
            if ($tgtType) { $tgtTypeNorm = $tgtType.Trim().ToLower() } else { $tgtTypeNorm = '' }
            if ($srcOp) { $srcOpNorm = $srcOp.Trim().ToLower() } else { $srcOpNorm = '' }
            if ($tgtOp) { $tgtOpNorm = $tgtOp.Trim().ToLower() } else { $tgtOpNorm = '' }
            Write-Verbose ("srcTypeNorm: '{0}', tgtTypeNorm: '{1}', srcOpNorm: '{2}', tgtOpNorm: '{3}'" -f $srcTypeNorm, $tgtTypeNorm, $srcOpNorm, $tgtOpNorm)
            if ($srcTypeNorm -eq 'version' -and $tgtTypeNorm -eq 'version') {
              if ((@('greaterthanorequal','greaterthan') -contains $srcOpNorm -or !$srcOpNorm) -and (@('greaterthanorequal','greaterthan') -contains $tgtOpNorm -or !$tgtOpNorm)) {
                Write-Verbose ("Problematic detection: Version check for file '{0}' in both apps (src: {1}, tgt: {2}, srcOp: {3}, tgtOp: {4})" -f $srcName, $srcTypeNorm, $tgtTypeNorm, $srcOpNorm, $tgtOpNorm)
                return $true
              }
            }
            if ($srcTypeNorm -eq 'exists' -and $tgtTypeNorm -eq 'exists') {
              Write-Verbose ("Problematic detection: Exists check for file '{0}' in both apps" -f $srcName)
              return $true
            }
          }
        } elseif ($isReg) {
          $srcReg = Normalize-Registry $src.keyPath $src.valueName
          $tgtReg = Normalize-Registry $tgt.keyPath $tgt.valueName
          Write-Verbose ("Comparing reg src: '{0}' tgt: '{1}'" -f $srcReg, $tgtReg)
          if ($srcReg -and $tgtReg -and $srcReg -eq $tgtReg) {
            $srcType = ($src.detectionType -as [string])
            $tgtType = ($tgt.detectionType -as [string])
            $srcOp = ($src.operator -as [string])
            $tgtOp = ($tgt.operator -as [string])
            ...
            if ($srcTypeNorm -eq 'version' -and $tgtTypeNorm -eq 'version') {
              if ((@('greaterthanorequal','greaterthan') -contains $srcOpNorm -or !$srcOpNorm) -and (@('greaterthanorequal','greaterthan') -contains $tgtOpNorm -or !$tgtOpNorm)) {
                Write-Verbose ("Problematic detection: Version check for registry '{0}' in both apps (src: {1}, tgt: {2}, srcOp: {3}, tgtOp: {4})" -f $srcReg, $srcTypeNorm, $tgtTypeNorm, $srcOpNorm, $tgtOpNorm)
                return $true
              }
            }
            if ($srcTypeNorm -eq 'exists' -and $tgtTypeNorm -eq 'exists') {
              Write-Verbose ("Problematic detection: Exists check for registry '{0}' in both apps" -f $srcReg)
              return $true
            }
          }
        }
  ...
  function Normalize-FileName {
    ...
    $n = $name -replace '\\','/' -replace '\s','' # slashes and whitespace
    $n = $n.ToLower()
    $n = [System.IO.Path]::GetFileName($n) # just filename
    return $n
  }
  ...
  function Read-RelationsJson {
    param([string]$IntuneAppRelationshipJSONFile)
      if ([string]::IsNullOrWhiteSpace($IntuneAppRelationshipJSONFile)) { throw 'Provide -IntuneAppRelationshipJSONFile' }
      if (-not (Test-Path -LiteralPath $IntuneAppRelationshipJSONFile)) { throw "File not found: $IntuneAppRelationshipJSONFile" }
      $Json = Get-Content -LiteralPath $IntuneAppRelationshipJSONFile -Raw -Encoding UTF8

      try { $data = $Json | ConvertFrom-Json }
    catch { throw "Failed to parse JSON: $($_.Exception.Message)" }
    if ($null -eq $data) { throw 'No data.' }
    if ($data -isnot [System.Collections.IEnumerable]) { throw 'Expected an array of relationships.' }
    return ,$data
  }
  ...
  function Get-DetectionCheck {
    ...
    if ($app.detectionRules) {
      ...
      foreach ($rule in $app.detectionRules) {
        $label = ''
        $tooltip = ''
        if ($rule.PSObject.Properties['scriptContent'] -and $rule.scriptContent) {
          $label = 'Custom Script'
          try {
            $decoded = [System.Text.Encoding]::UTF8.GetString([System.Convert]::FromBase64String($rule.scriptContent))
          } catch {
            $decoded = $rule.scriptContent
          }
          $tooltip = $decoded
        } elseif ($rule.PSObject.Properties['displayName'] -and $rule.displayName) {
          $label = HtmlEscape $rule.displayName
          $tooltip = ($rule | ConvertTo-Json -Depth 3)
        } else {
          $label = 'Detection Rule'
          $tooltip = ($rule | ConvertTo-Json -Depth 3)
        }
        # Format tooltip for readability (pretty-print JSON)
        $tooltipHtml = HtmlEscape $tooltip
        $rules += "<span class='tooltip'>$label<span class='tooltiptext'>$tooltipHtml</span></span>"
      }
      return $rules -join ', '
    }
    return ''
  ...
  function Build-TablesHtml {
    param([array]$Rels, [array]$AllApps)

    Write-Verbose "Build-TablesHtml called"
  
    $sup = $Rels |
      Where-Object { ([string]$_.'@odata.type') -like '*mobileAppSupersedence' } |
      Sort-Object @{Expression={[string]$_.sourceDisplayName}},
                  @{Expression={[string]$_.targetDisplayName}},
                  @{Expression={[string]$_.supersedenceType}}

    $dep = $Rels |
      Where-Object { ([string]$_.'@odata.type') -like '*mobileAppDependency' } |
      Sort-Object @{Expression={[string]$_.sourceDisplayName}},
                  @{Expression={[string]$_.targetDisplayName}},
                  @{Expression={[string]$_.dependencyType}}

    $supRows = foreach ($r in $sup) {
      $srcApp = $AllApps | Where-Object { $_.displayName -eq $r.sourceDisplayName }
      ...
      if ($tgtApp -and $tgtApp.detectionRules) {
        Write-Verbose ("Target detectionRules: {0}" -f ($tgtApp.detectionRules | ConvertTo-Json -Depth 5))
      } else {
        Write-Verbose "Target detectionRules: (none)"
      }
      $srcDetect = Get-DetectionCheck -AllApps $AllApps -AppName $r.sourceDisplayName
      $tgtDetect = Get-DetectionCheck -AllApps $AllApps -AppName $r.targetDisplayName
      $srcCellStyle = ''
      $tgtCellStyle = ''
      if (Test-ProblematicDetection $srcApp $tgtApp) {
        $srcCellStyle = " style='background-color:#ffcccc'"
        $tgtCellStyle = " style='background-color:#ffcccc'"
      }
      "<tr><td>$(HtmlEscape $r.sourceDisplayName)</td><td>$(HtmlEscape $r.sourceDisplayVersion)</td><td$srcCellStyle>$srcDetect</td><td>$(HtmlEscape $r.supersedenceType)</td><td>$(HtmlEscape $r.targetDisplayName)</td><td>$(HtmlEscape $r.targetDisplayVersion)</td><td$tgtCellStyle>$tgtDetect</td></tr>"
    }
    $depRows = foreach ($r in $dep) {
      $srcDetect = Get-DetectionCheck -AllApps $AllApps -AppName $r.sourceDisplayName
      $tgtDetect = Get-DetectionCheck -AllApps $AllApps -AppName $r.targetDisplayName
      "<tr><td>$(HtmlEscape $r.sourceDisplayName)</td><td>$srcDetect</td><td>$(HtmlEscape $r.dependencyType)</td><td>$(HtmlEscape $r.targetDisplayName)</td><td>$(HtmlEscape $r.targetDisplayVersion)</td><td>$tgtDetect</td></tr>"
    }

    $supTable = (@(
      '<h2>Supersedence</h2>',
      '<input id="supFilter" type="search" placeholder="Filter supersedence..." onkeyup="filterTable(''supTable'', this.value)" />',
      '<table id="supTable" class="grid">',
      '  <thead><tr><th>Source App</th><th>Source Version</th><th>Source Detection</th><th>Type</th><th>Target App</th><th>Target Version</th><th>Target Detection</th></tr></thead>',
      '  <tbody>'
    ) + $supRows + @(
      '  </tbody>',
      '</table>'
    ...
    $depTable = (@(
      '<h2>Dependencies</h2>',
      '<input id="depFilter" type="search" placeholder="Filter dependencies..." onkeyup="filterTable(''depTable'', this.value)" />',
      '<table id="depTable" class="grid">',
      '  <thead><tr><th>Parent App</th><th>Parent Detection</th><th>Dependency Type</th><th>Depends On</th><th>Dependency Version</th><th>Dependency Detection</th></tr></thead>',
      '  <tbody>'
    ) + $depRows + @(
      '  </tbody>',
      '</table>'
  ...
  function Build-ReportHtml {
    ...
    '  header { background-color: #444; color: white; padding: 10px; display: flex; align-items: center; }',
    '  header h1 { margin: 0; font-size: 24px; margin-right: 20px; }',
    '  header .additional-info { display: flex; flex-direction: column; align-items: flex-start; justify-content: center; }',
    '  header .additional-info p { margin: 0; line-height: 1.2; }',
    '  header .author-info { display: flex; flex-direction: column; align-items: flex-end; justify-content: center; margin-left: auto; }',
    '  header .author-info p { margin: 0; line-height: 1.2; }',
    '  header .author-info a { color: white; text-decoration: none; }',
    '  header .author-info a:hover { text-decoration: underline; }',
    '  table { border-collapse: collapse; width: 100%; text-align: left; }',
    '  table, table#TopTable { border: 2px solid #1C6EA4; background-color: #f7f7f4; }',
    '  table td, table th { border: 2px solid #AAAAAA; padding: 5px; }',
    '  table td { font-size: 15px; }',
    '  table th { font-size: 18px; font-weight: bold; color: #FFFFFF; background: #1C6EA4;',
    '    background: -moz-linear-gradient(top, #5592bb 0%, #327cad 66%, #1C6EA4 100%);',
    '    background: -webkit-linear-gradient(top, #5592bb 0%, #327cad 66%, #1C6EA4 100%);',
    '    background: linear-gradient(to bottom, #5592bb 0%, #327cad 66%, #1C6EA4 100%);',
    '  }',
    '  table#TopTable td, table#TopTable th { vertical-align: top; text-align: center; }',
    '  table thead th:first-child { border-left: none; }',
    '  table thead th span { font-size: 14px; margin-left: 4px; opacity: 0.7; }',
    '  table tfoot { font-size: 16px; font-weight: bold; color: #FFFFFF; background: #D0E4F5;',
    '    background: -moz-linear-gradient(top, #dcebf7 0%, #d4e6f6 66%, #D0E4F5 100%);',
    '    background: -webkit-linear-gradient(top, #dcebf7 0%, #d4e6f6 66%, #D0E4F5 100%);',
    '    background: linear-gradient(to bottom, #dcebf7 0%, #d4e6f6 66%, #D0E4F5 100%);',
    '    border-top: 2px solid #444444;',
    ...
    '  table tbody tr:nth-child(even) { background-color: #D0E4F5; }',
    '  select { font-family: "Courier New", monospace; }',
    '  footer { background-color: #444; color: white; padding: 10px; display: flex; align-items: center; justify-content: center; }',
    '  footer .creator-info { display: flex; flex-direction: row; align-items: center; margin-right: 20px; }',
    '  footer .creator-info p { line-height: 1.2; margin: 0; }',
    ...
    '  footer .company-logo { width: 100px; height: auto; margin: 0 20px; }',
    '  footer a { color: white; text-decoration: none; }',
    '  footer a:hover { text-decoration: underline; }',
    '  .filter-row { display: flex; align-items: center; }',
    '  .control-group { display: flex; flex-direction: column; align-items: flex-start; margin-right: 16px; }',
    '  .control-group label { font-weight: bold; }',
    '  .tooltip { position: relative; display: inline-block; cursor: pointer; color: inherit; }',
    '  .tooltip .tooltiptext { visibility: hidden; background-color: #555; color: #fff; text-align: left; border-radius: 6px; position: absolute; z-index: 1; bottom: 125%; left: 50%; margin-left: -60px; opacity: 0; transition: opacity 1s; white-space: pre; padding: 10px; }',
    ...
    $htmlLines = @(
      ...
      '</head>',
      '<body>',
      '  <header><h1>'+(HtmlEscape $Title)+'</h1></header>',
      '  <div class="wrap">',
      '    <div class="panel">' + $TablesHtml + '</div>',
      '    <div class="hint">Generated on ' + (Get-Date -Format 'yyyy-MM-dd HH:mm:ss K') + '</div>',
      '  </div>',
      '  ' + $js,
      '</body>',
      '</html>'
...
$rels = Read-RelationsJson -IntuneAppRelationshipJSONFile $IntuneAppRelationshipJSONFile
if (-not $rels) { throw 'No relationships to render.' }

$allAppsJson = Get-Content -LiteralPath $InIntuneAppsFile -Raw -Encoding UTF8
$allApps = $allAppsJson | ConvertFrom-Json

Write-Info ("Loaded {0} relationship records" -f $rels.Count)

$tables  = Build-TablesHtml  -Rels $rels -AllApps $allApps
$htmlOut = Build-ReportHtml -Title 'Intune App Relationships Report' -TablesHtml $tables

$parent = Split-Path -Parent $OutFile
if ($parent -and -not (Test-Path -LiteralPath $parent)) { [void](New-Item -ItemType Directory -Path $parent -Force) }

$null = Set-Content -Path $OutFile -Encoding UTF8 -Value $htmlOut

Write-Info "HTML written to: $OutFile"
if(-not $DoNotOpenReportAutomatically) {
    Write-Host "Opening created file:`n$OutFile`n"
    try {
        Invoke-Item "$OutFile"
    }
    catch {
        Write-Host "Error opening file automatically to browser. Open file manually:`n$OutFile`n" -ForegroundColor Red
    }
} else {
    Write-Host "`nNote! Parameter -DoNotOpenReportAutomatically specified. Report was not opened automatically to web browser`n" -ForegroundColor Yellow
}
